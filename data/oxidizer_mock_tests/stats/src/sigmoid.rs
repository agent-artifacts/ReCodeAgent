...
#[cfg(test)]
mod stats_sigmoid_harness {
    use super::*;
    #[test]
    fn sigmoid__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-montanaflynn-stats.Sigmoid.json",
            )
            .unwrap();
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(feature = "mock")]
                    { (sigmoid__with_callees_mocked(input_state.0)).unwrap() }
                    #[cfg(not(feature = "mock"))] { (sigmoid(input_state.0)).unwrap() }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
