...
pub fn min(input: Float64Data) -> Result<f64, anyhow::Error> {
    extern "C" {
        #[link_name = "stats_min__ground_truth"]
        fn min__foreign(_: JSONObject) -> JSONObject;
    }
    #[serde_as]
    #[derive(Serialize)]
    ...
    let foreign_execution = unsafe {
        de::<ForeignExecution>(min__foreign(ser(&input_state_in)))
    };
...
#[cfg(feature = "mock")]
pub fn min__with_callees_mocked(input: Float64Data) -> Result<f64, anyhow::Error> {
    let l = input.len();
    if l == 0 {
        return Err(anyhow!(EMPTY_INPUT_ERR.err.clone()));
    }
...
#[cfg(test)]
mod stats_min_harness {
    use super::*;
    #[test]
    fn min__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-montanaflynn-stats.Min.json",
            )
            .unwrap();
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(feature = "mock")]
                    { (min__with_callees_mocked(input_state.0)).unwrap() }
                    #[cfg(not(feature = "mock"))] { (min(input_state.0)).unwrap() }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
