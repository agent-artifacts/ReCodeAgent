...
#[cfg(test)]
mod gohistogram_ewma_harness {
    use super::*;
    #[test]
    fn ewma__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-VividCortex-gohistogram.ewma.json",
            )
            .unwrap();
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(not(feature = "mock"))]
                    { (ewma(input_state.0, input_state.1, input_state.2)).unwrap() }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
...
#[cfg(test)]
mod gohistogram_WeightedHistogram_interoperation_tests {
    use super::*;
    extern "C" {
        #[link_name = "gohistogram_WeightedHistogram_roundtrip"]
        fn WeightedHistogram__roundtrip(_: JSONObject) -> JSONObject;
    }
    #[test]
    fn WeightedHistogram__weak__interoperation() {
                ...
                if let Ok(entry) = entry {
                    if entry
                        ...
                        .starts_with(
                            &format!(
                                "(*{}).",
                                "github.com-VividCortex-gohistogram.WeightedHistogram"
                            ),
                        )
                        || entry
                            .file_name()
                            ...
                            )
                    {
                        ...
                        for unittest in unittests {
                            ...
                            let obj_once = serde_json::to_value(
                                    serde_json::from_value::<WeightedHistogram>(obj).unwrap(),
                                )
                                .unwrap();
...
#[cfg(test)]
mod gohistogram_weighted_histogram___scale_down_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_scale_down__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).scaleDown.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
        ...
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<WeightedHistogram>, usize);
        #[serde_as]
        #[derive(Serialize)]
        struct OutputState;
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    {
                        (&mut *input_state.0)
                            .scale_down__with_callees_mocked(input_state.1)
                    }
                    #[cfg(not(feature = "mock"))]
                    { (&mut *input_state.0).scale_down(input_state.1) }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
    ...
    #[test]
    fn WeightedHistogram_scale_down__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<WeightedHistogram>, usize);
        #[serde_as]
...
#[cfg(test)]
mod gohistogram_weighted_histogram___trim_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_trim__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).trim.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
        ...
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<WeightedHistogram>);
        #[serde_as]
        #[derive(Serialize)]
        struct OutputState;
        for execution in unittests {
    ...
    #[test]
    fn WeightedHistogram_trim__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<WeightedHistogram>);
        #[serde_as]
...
#[cfg(test)]
mod gohistogram_weighted_histogram___add_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_add__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).Add.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
    ...
    #[test]
    fn WeightedHistogram_add__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(
            Box<WeightedHistogram>,
...
#[cfg(test)]
mod gohistogram_weighted_histogram__cdf_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_cdf__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).CDF.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
    ...
    #[test]
    fn WeightedHistogram_cdf__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(
            Box<WeightedHistogram>,
...
#[cfg(test)]
mod gohistogram_weighted_histogram___count_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_count__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).Count.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
    ...
    #[test]
    fn WeightedHistogram_count__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<WeightedHistogram>);
        #[serde_as]
...
#[cfg(test)]
mod gohistogram_weighted_histogram___mean_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_mean__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).Mean.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
    ...
    #[test]
    fn WeightedHistogram_mean__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<WeightedHistogram>);
        #[serde_as]
...
#[cfg(test)]
mod gohistogram_weighted_histogram___quantile_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_quantile__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).Quantile.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(not(feature = "mock"))]
                    { ((&*input_state.0).quantile(input_state.1)).unwrap() }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
    ...
    #[test]
    fn WeightedHistogram_quantile__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(
            Box<WeightedHistogram>,
...
#[cfg(test)]
mod gohistogram_weighted_histogram___variance_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_variance__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-VividCortex-gohistogram.WeightedHistogram).Variance.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
    ...
    #[test]
    fn WeightedHistogram_variance__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<WeightedHistogram>);
        #[serde_as]
...
#[cfg(test)]
mod gohistogram_new_weighted_histogram_harness {
    use super::*;
    #[test]
    fn WeightedHistogram_new__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-VividCortex-gohistogram.NewWeightedHistogram.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(feature = "mock")]
                    {
                        (WeightedHistogram::new__with_callees_mocked(
                            input_state.0,
                            input_state.1,
                        ))
                            .unwrap()
                    ...
                    #[cfg(not(feature = "mock"))]
                    { (WeightedHistogram::new(input_state.0, input_state.1)).unwrap() }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
    ...
    #[test]
    fn WeightedHistogram_new__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(
            usize,
