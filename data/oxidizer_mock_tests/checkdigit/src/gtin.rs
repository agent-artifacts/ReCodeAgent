...
#[cfg(test)]
mod checkdigit_gtin_interoperation_tests {
    use super::*;
    extern "C" {
        #[link_name = "checkdigit_gtin_roundtrip"]
        fn gtin__roundtrip(_: JSONObject) -> JSONObject;
    }
    #[test]
    fn gtin__weak__interoperation() {
            ...
            for entry in entries {
                if let Ok(entry) = entry {
                    if entry
                        ...
                        .unwrap()
                        .starts_with(
                            &format!("(*{}).", "github.com-osamingo-checkdigit.gtin"),
                        )
                        || entry
                            .file_name()
                            .to_str()
                            ...
                            )
                    {
                        ...
                        for unittest in unittests {
                            ...
                            let obj_once = serde_json::to_value(
                                    serde_json::from_value::<gtin>(obj).unwrap(),
                                )
                                .unwrap();
                            let obj_twice = serde_json::to_value(
                                    serde_json::from_value::<gtin>(obj_once.clone()).unwrap(),
                                )
                                .unwrap();
...
#[cfg(test)]
mod checkdigit_gtin___generate_harness {
    use super::*;
    #[test]
    fn gtin_generate__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-osamingo-checkdigit.gtin).Generate.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        ...
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<gtin>, Box<str>);
        #[serde_as]
        #[derive(Serialize)]
    ...
    #[test]
    fn gtin_generate__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<gtin>, Box<str>);
        #[serde_as]
...
#[cfg(test)]
mod checkdigit_gtin___verify_harness {
    use super::*;
    #[test]
    fn gtin_verify__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(github.com-osamingo-checkdigit.gtin).Verify.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
    ...
    #[test]
    fn gtin_verify__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<gtin>, Box<str>);
        #[serde_as]
