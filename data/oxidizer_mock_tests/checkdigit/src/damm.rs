...
#[cfg(test)]
mod checkdigit_damm_interoperation_tests {
    use super::*;
    extern "C" {
        #[link_name = "checkdigit_damm_roundtrip"]
        fn Damm__roundtrip(_: JSONObject) -> JSONObject;
    }
    #[test]
    fn Damm__weak__interoperation() {
            ...
            for entry in entries {
                if let Ok(entry) = entry {
                    if entry
                        ...
                        .unwrap()
                        .starts_with(
                            &format!("(*{}).", "github.com-osamingo-checkdigit.damm"),
                        )
                        || entry
                            .file_name()
                            .to_str()
                            ...
                            )
                    {
                        ...
                        for unittest in unittests {
                            ...
                            let obj_once = serde_json::to_value(
                                    serde_json::from_value::<Damm>(obj).unwrap(),
                                )
                                .unwrap();
                            let obj_twice = serde_json::to_value(
                                    serde_json::from_value::<Damm>(obj_once.clone()).unwrap(),
                                )
                                .unwrap();
...
#[cfg(test)]
mod checkdigit_damm___generate_harness {
    use super::*;
    #[test]
    fn Damm_generate__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(*github.com-osamingo-checkdigit.damm).Generate.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        ...
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<Damm>, Box<str>);
        #[serde_as]
        #[derive(Serialize)]
    ...
    #[test]
    fn Damm_generate__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<Damm>, Box<str>);
        #[serde_as]
...
#[cfg(test)]
mod checkdigit_damm___verify_harness {
    use super::*;
    #[test]
    fn Damm_verify__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/(github.com-osamingo-checkdigit.damm).Verify.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
    ...
    #[test]
    fn Damm_verify__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<Damm>, Box<str>);
        #[serde_as]
