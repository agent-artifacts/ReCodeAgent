...
#[cfg(test)]
mod go_edlib_matching_index_harness {
    use super::*;
    #[test]
    fn matching_index__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-hbollon-go-edlib.matchingIndex.json",
            )
            .unwrap();
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(feature = "mock")]
                    {
                        (matching_index__with_callees_mocked(
                            &*input_state.0,
                            &*input_state.1,
                            input_state.2,
                        ))
                    ...
                    #[cfg(not(feature = "mock"))]
                    {
                        (matching_index(&*input_state.0, &*input_state.1, input_state.2))
                            .unwrap()
                    }
                }),
            );
    ...
    #[test]
    fn matching_index__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<str>, Box<str>, usize);
        #[serde_as]
...
#[cfg(test)]
mod go_edlib_strings_similarity_harness {
    use super::*;
    #[test]
    fn strings_similarity__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-hbollon-go-edlib.StringsSimilarity.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
        ...
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<str>, Box<str>, Algorithm);
        #[serde_as]
        #[derive(Serialize)]
        struct OutputState(
            #[serde_as(as = "crate :: interoperation_utils :: MyFloat32")]
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(feature = "mock")]
                    {
                        (strings_similarity__with_callees_mocked(
                            &*input_state.0,
                            &*input_state.1,
                            input_state.2,
                        ))
    ...
    #[test]
    fn strings_similarity__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<str>, Box<str>, Algorithm);
        #[serde_as]
