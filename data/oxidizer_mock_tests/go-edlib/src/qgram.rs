...
#[cfg(test)]
mod go_edlib_qgram_distance_custom_ngram_harness {
    use super::*;
    #[test]
    fn qgram_distance_custom_ngram__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-hbollon-go-edlib.QgramDistanceCustomNgram.json",
            )
            .unwrap();
        ...
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<HashMap<String, i32>>, Box<HashMap<String, i32>>);
        #[serde_as]
        #[derive(Serialize)]
        struct OutputState(i32);
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(feature = "mock")]
                    {
                        qgram_distance_custom_ngram__with_callees_mocked(
                            &*input_state.0,
                            &*input_state.1,
                        )
                    }
                    #[cfg(not(feature = "mock"))]
                    { qgram_distance_custom_ngram(&*input_state.0, &*input_state.1) }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
    ...
    #[test]
    fn qgram_distance_custom_ngram__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<HashMap<String, i32>>, Box<HashMap<String, i32>>);
        #[serde_as]
...
#[cfg(test)]
mod go_edlib_qgram_similarity_harness {
    use super::*;
    #[test]
    fn qgram_similarity__unit_test() {
        let unittests_file: std::fs::File = std::fs::File::open(
                "./exec-snapshots/github.com-hbollon-go-edlib.QgramSimilarity.json",
            )
            .unwrap();
        let unittests_reader = std::io::BufReader::new(unittests_file);
        let unittests: Vec<ExecutionData> = serde_json::from_reader(unittests_reader)
        ...
        for execution in unittests {
            ...
            let return_value = std::panic::catch_unwind(
                std::panic::AssertUnwindSafe(|| {
                    ...
                    #[cfg(feature = "mock")]
                    {
                        qgram_similarity__with_callees_mocked(
                            &*input_state.0,
                            &*input_state.1,
                            input_state.2,
                        )
                    ...
                    #[cfg(not(feature = "mock"))]
                    { qgram_similarity(&*input_state.0, &*input_state.1, input_state.2) }
                }),
            );
            match return_value {
                Ok(mut return_value) => {
    ...
    #[test]
    fn qgram_similarity__signature_check() {
        #[serde_as]
        #[derive(Serialize, Deserialize)]
        struct InputState(Box<str>, Box<str>, usize);
        #[serde_as]
