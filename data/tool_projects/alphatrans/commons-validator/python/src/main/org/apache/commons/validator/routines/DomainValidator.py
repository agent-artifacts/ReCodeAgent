...
class DomainValidator:
    ...
    __genericTLDsPlus: typing.List[typing.List[str]] = None  # LLM could not translate this field

    __inUse: bool = False
    __LOCAL_TLDS: typing.List[typing.List[str]] = [
    ...
    __GENERIC_TLDS: typing.Tuple[str, ...] = (
    ...
    __INFRASTRUCTURE_TLDS: typing.List[typing.List[str]] = [["arpa"]]  # internet infrastructure
    ...
    __EMPTY_STRING_ARRAY: typing.List[typing.List[str]] = None  # LLM could not translate this field

    __MAX_DOMAIN_LENGTH: int = 253
    __localTLDsPlus: typing.List[typing.List[str]] = __EMPTY_STRING_ARRAY
    __localTLDsMinus: typing.List[typing.List[str]] = __EMPTY_STRING_ARRAY
    __genericTLDsMinus: typing.List[typing.List[str]] = __EMPTY_STRING_ARRAY
    __countryCodeTLDsMinus: typing.List[typing.List[str]] = __EMPTY_STRING_ARRAY
    __countryCodeTLDsPlus: typing.List[typing.List[str]] = __EMPTY_STRING_ARRAY
    ...
    def unicodeToASCII(input_: str) -> str:
        ...
        try:
            ...
            if length == 0:  # check there is a last character
                return input_
            lastChar = input_[length - 1]  # fetch original last char
            if lastChar in ["\u002e", "\u3002", "\uff0e", "\uff61"]:
    ...
    def getOverrides(self, table: ArrayType) -> typing.List[typing.List[str]]:
        ...
        return array.copy() if array is not None else None
    ...
    @staticmethod
    def updateTLDOverride(table: ArrayType, tlds: typing.List[typing.List[str]]) -> None:
    ...
    def isAllowLocal(self) -> bool:
        return self._DomainValidator__allowLocal

    def isValidLocalTld(self, lTld: str) -> bool:
        key = self.__chompLeadingDot(DomainValidator.unicodeToASCII(lTld).lower())

        # Flatten the nested lists to simple lists
        local_tlds_flat = [item for sublist in DomainValidator.__LOCAL_TLDS for item in sublist]
        mylocalTLDsPlus_flat = (
            [item for sublist in self.mylocalTLDsPlus for item in sublist] if self.mylocalTLDsPlus else []
        )
        mylocalTLDsMinus_flat = (
            [item for sublist in self.mylocalTLDsMinus for item in sublist] if self.mylocalTLDsMinus else []
        )

        return (
            DomainValidator.__arrayContains(local_tlds_flat, key)
            or DomainValidator.__arrayContains(mylocalTLDsPlus_flat, key)
        ) and not DomainValidator.__arrayContains(mylocalTLDsMinus_flat, key)

    def isValidCountryCodeTld(self, ccTld: str) -> bool:
        key = self.__chompLeadingDot(DomainValidator.unicodeToASCII(ccTld).lower())
        return (
            DomainValidator._DomainValidator__arrayContains(DomainValidator._DomainValidator__COUNTRY_CODE_TLDS, key)
            or DomainValidator._DomainValidator__arrayContains(self.mycountryCodeTLDsPlus, key)
        ) and not DomainValidator._DomainValidator__arrayContains(self.mycountryCodeTLDsMinus, key)
    ...
    def isValidTld(self, tld: str) -> bool:
        if self._DomainValidator__allowLocal and self.isValidLocalTld(tld):
            return True
        return self.isValidInfrastructureTld(tld) or self.isValidGenericTld(tld) or self.isValidCountryCodeTld(tld)
    ...
    def isValid(self, domain: str) -> bool:
        ...
        if len(domain) > DomainValidator._DomainValidator__MAX_DOMAIN_LENGTH:
            return False
        groups = self._DomainValidator__domainRegex.match(domain)
        if groups is not None and len(groups) > 0:
            return self.isValidTld(groups[0])
        return self._DomainValidator__allowLocal and self._DomainValidator__hostnameRegex.isValid(domain)

    def __init__(self, constructorId: int, items: typing.List[Item], allowLocal: bool) -> None:
        if constructorId == 0:

            self._DomainValidator__allowLocal = allowLocal
            ...
            for item in items:
                copy = [None] * len(item.values)
                for i in range(len(item.values)):
                    copy[i] = item.values[i].lower()
                copy.sort()
...
class IDNBUGHOLDER:
    ...
    def initialize_fields() -> None:
        IDNBUGHOLDER.__IDN_TOASCII_PRESERVES_TRAILING_DOTS: bool = IDNBUGHOLDER.__keepsTrailingDot()
...
class LazyHolder:
    ...
    __DOMAIN_VALIDATOR_WITH_LOCAL: DomainValidator = None  # LLM could not translate this field
