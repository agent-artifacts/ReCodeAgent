...
func NewDefaultRule() *RuleDefault {
	return NewRule()
}
...
func NewDefaultLanguage() *LanguageDefault {
	return NewLanguage()
}
...
// use TextRank.
func NewDefaultAlgorithm() *AlgorithmDefault {
	return NewAlgorithmDefault()
}
...
func NewChainAlgorithm() *AlgorithmChain {
	return NewAlgorithmChain()
}
...
func (textRank *TextRank) Populate(
	...
	parsedText := TokenizeText(text, rule)

	for _, sentence := range parsedText.GetSentences() {
		TextToRank(sentence, lang, textRank.rank)
	}
}
...
func (textRank *TextRank) Ranking(algorithm Algorithm) {
	Calculate(textRank.rank, algorithm)
}
...
func (textRank *TextRank) GetRankData() *Rank {
...
func (textRank *TextRank) FindPhrases() []Phrase {
	return FindPhrases(textRank.rank)
}
...
// quantities of word.
func (textRank *TextRank) FindSingleWords() []SingleWord {
	return FindSingleWords(textRank.rank)
}
...
// phrases from 1 to 0.
func (textRank *TextRank) FindSentencesByRelationWeight(
	limit int,
) []Sentence {

	return FindSentences(textRank.rank, ByRelation, limit)
}
...
func (textRank *TextRank) FindSentencesByWordQtyWeight(
	limit int,
) []Sentence {

	return FindSentences(textRank.rank, ByQty, limit)
}
...
func (textRank *TextRank) FindSentencesByPhraseChain(
	phrases []string,
) []Sentence {

	return FindSentencesByPhrases(textRank.rank, phrases)
}
...
func (textRank *TextRank) FindSentencesFrom(
	sentenceID int,
	limit int,
) []Sentence {

	return FindSentencesFrom(textRank.rank, sentenceID, limit)
}
