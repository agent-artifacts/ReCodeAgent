...
func assertTheGnomeTestTextDefault(t *testing.T, textRank *TextRank) {
	...
	phrases := textRank.FindPhrases()
	max := len(mostPopulars) - 1

	for i := 0; i < max; i++ {
		found := func(ph Phrase) bool {
	...
	singleWords := textRank.FindSingleWords()
	...
	sentencesByQtyWeight := textRank.FindSentencesByWordQtyWeight(6)
	...
	sentencesByRelWeight := textRank.FindSentencesByRelationWeight(6)
	...
	sentencesByPhrase := textRank.FindSentencesByPhraseChain([]string{
		"gnome",
		"shell",
		"extension",
	})
	...
	sentenceIDStart := 10
	foundSentences := textRank.FindSentencesFrom(sentenceIDStart, 3)
